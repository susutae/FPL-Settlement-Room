...
async function loadLive(proxy:string){
  const first=await getJson(`/api/leagues-classic/${LEAGUE_ID}/standings/?page_standings=1`,proxy);const pages=[first];let page=2;
  ...
  const standings=pages.flatMap(p=>p.standings?.results??[]).slice(0,15);if(!standings.length)throw new Error("No league entries returned");
  const managers:Manager[]=standings.map((entry:Record<string,number|string>)=>({id:Number(entry.entry),name:String(entry.player_name),team:String(entry.entry_name),overallPoints:Number(entry.total??0),overallRank:null,weeklyTotal:0}));
  const histories=await Promise.all(managers.map(m=>getJson(`/api/entry/${m.id}/history/`,proxy)));const maxGw=Math.max(0,...histories.flatMap(h=>(h.current??[]).map((x:EntryHistory)=>x.event)));
  ...
  for(let gw=1;gw<=maxGw;gw++){
    ...
    const raw=managers.map((manager,i)=>{const history=historyMaps[i].get(gw);const fallback=picks[i]?.entry_history;const hit=Number(history?.event_transfers_cost??fallback?.event_transfers_cost??0);const currentTotal=Number(history?.total_points??fallback?.total_points??manager.overallPoints);const previousTotal=Number(historyMaps[i].get(gw-1)?.total_points??0);const net=history?currentTotal-previousTotal:Number(fallback?.points??0)-hit;return{...manager,gw,hit,net,gross:net+hit,bench:Number(history?.points_on_bench??fallback?.points_on_bench??0),overallPoints:currentTotal,overallRank:history?.overall_rank??manager.overallRank};}).sort((a,b)=>b.net-a.net||b.bench-a.bench||a.name.localeCompare(b.name));
  ...
  managers.forEach((m,i)=>{const latest=[...historyMaps[i].values()].at(-1);m.weeklyTotal=running.get(m.id)??0;m.overallPoints=latest?.total_points??m.overallPoints;m.overallRank=latest?.overall_rank??null;});return{managers,weeks,leagueName:first.league?.name||`League ${LEAGUE_ID}`};
}

export default function Home(){
  const seed=useMemo(demoData,[]);const[managers,setManagers]=useState(seed.managers);const[weeks,setWeeks]=useState<WeekRow[][]>(seed.weeks);const[selectedGw,setSelectedGw]=useState(seed.weeks.length);const[state,setState]=useState<LoadState>("loading");const[leagueName,setLeagueName]=useState("Weekly Treat League");const[proxy,setProxy]=useState("");const[settingsOpen,setSettingsOpen]=useState(false);const[lastUpdated,setLastUpdated]=useState("—");const[cupWinner,setCupWinner]=useState<number|null>(null);const[cupRunnerUp,setCupRunnerUp]=useState<number|null>(null);
  async function refresh(proxyOverride?:string){const nextProxy=proxyOverride??proxy;setState("loading");try{const live=await loadLive(nextProxy.trim());setManagers(live.managers);setWeeks(live.weeks);setLeagueName(live.leagueName);setSelectedGw(Math.max(1,live.weeks.length));setState("live");setLastUpdated(new Date().toLocaleString([],{dateStyle:"medium",timeStyle:"short"}));}catch(error){console.warn("FPL live data unavailable; showing demo data.",error);setManagers(seed.managers);setWeeks(seed.weeks);setSelectedGw(seed.weeks.length);setState("demo");setLastUpdated("Demo snapshot");}}
  useEffect(()=>{const saved=window.localStorage.getItem("fpl-proxy")??"";setProxy(saved);void refresh(saved);/* eslint-disable-next-line react-hooks/exhaustive-deps */},[]);
  ...
  return <main>
    ...
    <section className="score-section"><div className="section-heading"><div><p className="eyebrow">WEEKLY LEDGER</p><h2>Gameweek {selectedGw}</h2></div><div className="gw-control"><button onClick={()=>setSelectedGw(g=>Math.max(1,g-1))} disabled={selectedGw<=1} aria-label="Previous gameweek">←</button><label><span>SELECT GAMEWEEK</span><select value={selectedGw} onChange={e=>setSelectedGw(Number(e.target.value))}>{weeks.map((_,i)=><option value={i+1} key={i}>GW {i+1}</option>)}</select></label><button onClick={()=>setSelectedGw(g=>Math.min(weeks.length,g+1))} disabled={selectedGw>=weeks.length} aria-label="Next gameweek">→</button></div></div>
      ...
      <div className="legend"><span><i className="win-dot"/>Top 4 paid</span><span><i className="loss-dot"/>Bottom 4 charged</span><span>Net = GW points − transfer hit, including negative results</span><span>Updated {lastUpdated}</span></div></section>
    ...
    {settingsOpen&&<div className="modal-backdrop" onMouseDown={()=>setSettingsOpen(false)}><div className="modal" role="dialog" aria-modal="true" aria-labelledby="settings-title" onMouseDown={e=>e.stopPropagation()}><button className="modal-close" onClick={()=>setSettingsOpen(false)} aria-label="Close settings">×</button><p className="eyebrow">CONNECTION</p><h2 id="settings-title">Data settings</h2><p>Leave this blank to use the included FPL relay. If your host blocks that relay, enter a trusted CORS proxy prefix. The encoded API URL will be appended automatically.</p><label className="proxy-field"><span>Custom proxy prefix</span><input value={proxy} onChange={e=>setProxy(e.target.value)} placeholder="https://your-proxy.example/?url="/></label><div className="modal-actions"><button className="secondary" onClick={()=>{setProxy("");window.localStorage.removeItem("fpl-proxy")}}>Use built-in relay</button><button className="primary" onClick={()=>{window.localStorage.setItem("fpl-proxy",proxy.trim());setSettingsOpen(false);void refresh(proxy.trim())}}>Save & retry</button></div></div></div>}
  </main>;
}
